...
#         # Now publish the text marker. 
#         # For example, place the text above the origin (z=0.2) so it’s readable.
#         text_str = f"NEXT FORCE: {self.cable_force:.2f} N"
#         self.publish_text_marker(text_str, x_pos=0.0, y_pos=0.0, z_pos=0.2, scale=0.05, color=(1.0, 1.0, 1.0))
...
#         marker.id = 0
#         marker.type = Marker.TEXT_VIEW_FACING  # This makes the marker display text
#         marker.action = Marker.ADD

#         # Position of the text (in meters, just like other markers)
#         marker.pose.position.x = float(x_pos)
#         marker.pose.position.y = float(y_pos)
#         marker.pose.position.z = float(z_pos)
#         marker.pose.orientation.w = 1.0

#         # The scale.x is the height of an uppercase "A" in meters
