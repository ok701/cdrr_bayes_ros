...
class BayesOptHWNode(Node):
    ...
    def stop_recording_and_optimize(self):
        ...
        # 1) Compute cost (only after run ends)
        ...
        # Publish these final scores (you may choose to use the same /score topic or a different one).
        self.publish_realtime_scores(self.last_run_strength, self.last_run_accuracy)

        # 3) Bayesian optimization (if not reached max_runs)
        self.run_count += 1
    ...
    # ------------------------------------------------------------------
    #            Bayesian Optimization Helpers (unchanged)
    # ------------------------------------------------------------------
