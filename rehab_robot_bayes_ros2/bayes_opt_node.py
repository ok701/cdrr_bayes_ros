...
#     - Now also publishes real-time strength_score and accuracy_score at 10 Hz.
#     """
...
#         """
#         Periodic tasks handled here at self.freq (e.g., 10 Hz).
#         We'll publish real-time strength_score and accuracy_score here.
#         """
#         # --- Real-time scores ---
#         strength_score, accuracy_score = self.compute_realtime_scores()

#         # DEBUG LOG: Check what we are about to publish
#         # self.get_logger().info(
#         #     f"[DEBUG main_loop_callback] Real-time strength={strength_score:.2f}, accuracy={accuracy_score:.2f}"
#         # )

#         # Publish these real-time scores
#         self.publish_realtime_scores(strength_score, accuracy_score)

#         # You can do other periodic tasks here if needed.
...
#         # self.publish_realtime_scores(0.0, 0.0)  # Reset real-time scores
...
#         # 1) Compute cost from the current run's data
...
#         # Publish these final scores for this run
#         self.publish_realtime_scores(self.last_run_strength, self.last_run_accuracy)

#         # Increment how many times we've repeated this particular force
#         self.current_repeat_count += 1
#         self.get_logger().info(f"   - Current repeat count: {self.current_repeat_count}/{self.repeat_per_force}")

#         # 3) Check if we need to repeat the same force
#         if self.current_repeat_count < self.repeat_per_force:
#                 # We still have repeats left, so publish the same force again
#                 # No Beysian iteration yet.
#                 self.get_logger().info(f"   - Repeating the same force; no new optimization step.")
#                 self.publish_ref_force(self.next_cable_force)
#         else:
#             # Finished all repeats for this force, now do Bayesian optimization
#             self.current_repeat_count = 0

#             # Bayesian optimization (if not reached max_runs)
#             self.run_count += 1
...
#                 # Publish new cable force for next run
#                 self.publish_ref_force(self.next_cable_force)
#             else:
#                 # If we've done enough runs, publish a default (e.g., 0).
...
#         after a full run has ended.
#         """
#         if len(error_list) == 0:
#             return 0.0, 0.0, 0.0

#         # Same logic from your example
...
#     # ------------------------------------------------------------------
#     #                Real-Time Score Computation (NEW)
#     # ------------------------------------------------------------------
#     def compute_realtime_scores(self):
#         """
#         Compute and return (strength_score, accuracy_score) in real time.
#         - Only do it if we're collecting data, otherwise return (0,0).
#         - Example approach: use the running mean error from ee_error_list,
#           and the current self.next_cable_force to compute strength.
#         """

#         # If not collecting data, return the last run's final scores
#         if not self.collecting_data or len(self.ee_error_list) == 0:
#             return self.last_run_strength, self.last_run_accuracy

#         # Real-time approximate => use mean error so far
#         mean_error = np.mean(self.ee_error_list)

#         # Reuse the same formulas from compute_cost_from_data, except we skip cost_val
...
#         """
#         Publishes the real-time strength_score & accuracy_score at 10 Hz.
#         These are simply approximate real-time versions (NOT the final after-run cost).
#         """
#         msg = Float32MultiArray()
#         msg.data = [strength_score, accuracy_score]
#         self.score_pub.publish(msg)
#         # You can comment this out or reduce the frequency if it’s too chatty
#         # self.get_logger().info(
#         #     f"[Real-Time] strength={strength_score:.2f}, accuracy={accuracy_score:.2f}"
#         # )
...
#         # 1) Evaluate EI over the ENTIRE domain
#         global_candidates = np.linspace(self.lower_bound, 
#                                         self.upper_bound, 200)  # or more points
#         global_candidates = global_candidates.reshape(-1, 1)
#         ei_values = self.expected_improvement(global_candidates, X_data, y_data, gp)
        
#         # 2) Find the global maximum of EI
#         best_idx_global = np.argmax(ei_values)
#         F_global_best = float(global_candidates[best_idx_global, 0])
        
#         # # 3) Clamp the next step to be within +/- step_size of the last force
#         # if len(self.X) > 0:
#         #     F_prev = self.X[-1]
#         # else:
#         #     F_prev = 0.0
...
#         # # "Clip" the best global candidate to respect the max step
#         # F_next = max(F_prev - step_size, min(F_prev + step_size, F_global_best))

#         F_next = F_global_best  # No clamping step size for now
...
class BayesOptHWNode(Node):
    ...
    def stop_recording_and_optimize(self):
        ...
        # (b) Remove outliers
        filtered_costs = self.remove_outliers(self.current_force_costs, k=1)
    ...
    def propose_next_force(self, X_data, y_data):
        ...
        ei_values = self.expected_improvement(global_candidates, X_data, y_data, self.gp)
