...
class LinearEnvSimVizNode(Node):
    ...
    A ROS2 node that:
      - Simulates a 1D linear environment in mm in real-time.
      - Accepts an external force in Newtons via /ref_force.
      - Publishes mes_pos and ref_pos to be used by a Bayesian optimization node.
      - Visualizes the environment in RViz (converted to meters for the marker).
      - Resets the simulation on a /trigger signal:
          - 0 -> 1: Start the simulation.
          - 1 -> 0: Reset the simulation and go back to x = 0.
    ...
    def __init__(self, freq=50.0, frame_id='world'):
        ...
        # Renamed from /manipulated_var to /ref_force
        self.ref_force_sub = self.create_subscription(
            Int32, '/ref_force', self.ref_force_callback, 10
        )

        # Publish mes_pos (environment position) and ref_pos (trajectory reference) in mm
    ...
    def update_and_visualize(self):
        """
        1. Update the simulation dynamics (in mm, mm/s, mm/s^2).
        2. Publish mes_pos (self.x) and ref_pos (x_ref) in mm.
        3. Visualize in RViz (convert mm to m for markers).
        """
        if not self.simulation_active:
            return
        ...
        # Net force in N = cable_force - damping * velocity + human_force
        net_force = self.cable_force - (self.b * self.x_dot) + human_force

        # Acceleration in mm/s^2 = 1000 * net_force / mass (since net_force is in N, mass in kg)
        x_ddot = 1000.0 * (net_force / self.mass)
        ...
        # Publish mes_pos and ref_pos in mm
    ...
    def publish_marker(self, topic, position_m, scale_m, color):
        """
        Publish a sphere Marker in RViz (all distances in meters).
        """
        marker = Marker()
        marker.header.frame_id = self.frame_id
        marker.header.stamp = self.get_clock().now().to_msg()
